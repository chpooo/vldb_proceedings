...
<xsl:template match="period">
	...
	<xsl:for-each select="./slot">
		...
		<h3 class="location">
            ...
            <img src="lib/map.png" alt="map" title="Show map" class="paper_controls">
		...
		<xsl:for-each select="./paper">
			<div id="booklet_data" class="booklet_paper_folded">
                <img src="lib/pdf.png" alt="pdf" title="Download PDF" class="paper_controls">
                        <xsl:attribute name="onclick">TryOpen('<xsl:value-of select="link"/>')</xsl:attribute>
                </img>
                <xsl:if test="abstract">
                    <img src="lib/txt.png" alt="txt" title="Show Abstract" class="paper_controls" onclick="foldContent(this)"/>
                </xsl:if>
